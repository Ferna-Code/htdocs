	        ...
	        <div class="formulario">
	         ...
	         <!--Completar el Código que se requerirá a continuación--> 	
					...
					if (isset($_POST['actualiza'])) {
						$idEliminar = $_POST['seleccionar'];
						$stock = $_POST['stock'];
						
						$queryUpdate = "UPDATE productos SET stock = ? WHERE cod_producto = ?";
						$prepareUpdate = mysqli_prepare($conexion, $queryUpdate);
						mysqli_stmt_bind_param($prepareUpdate, "ss", $stock, $idEliminar);
				...
				<?php 
					...
					if (isset($_POST['modificar'])) {
						$idProducto = $_POST['seleccionar2'];
						$descripcion = $_POST['descripcion'];
						$proveedor = $_POST['proveedor'];
						$fecha = $_POST['fecha'];

						$queryUpdate2 = "UPDATE productos SET descripcion = ?, proveedor = ?, fecha_ingreso = ? WHERE cod_producto = ?";
						$prepareUpdate2 = mysqli_prepare($conexion, $queryUpdate2);
						mysqli_stmt_bind_param($prepareUpdate2,"ssss", $descripcion, $proveedor, $fecha, $idProducto);
						mysqli_stmt_execute($prepareUpdate2);
						mysqli_stmt_close($prepareUpdate2);
						header('Location: mod_producto.php');
						exit();
